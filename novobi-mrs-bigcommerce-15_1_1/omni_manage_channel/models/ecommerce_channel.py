...
class EcommerceChannel(models.Model):
    ...
    def _prepare_data(self):
        user = self.env.user
        installed_apps = self.env["ir.module.module"].search_read(
            [
                ("state", "in", ["installed", "to upgrade", "to remove"]),
            ],
            ["name", "installed_version", "latest_version", "published_version", "author", "state"],
        )
        get_param = self.env["ir.config_parameter"].sudo().get_param

        db_uuid = get_param("database.uuid")
        db_name = self._cr.dbname
        db_create_date = get_param("database.create_date")
        web_base_url = get_param("web.base.url")
        company = (
            user.partner_id.company_id
            and user.partner_id.company_id.read(["name", "email", "phone"])[0]
            or ""
        )
        user_count = self.env["res.users"].search_count([("share", "=", False)])
        channels = self.env["ecommerce.channel"].search([("active", "=", True)])
        channel_datas = []
        for channel in channels:
            channel_dashboard_data = channel.get_dashboard_datas()
            channel_datas.append(
                {
                    "name": channel.name,
                    "platform": channel.platform,
                    "last_30_days_sales": channel_dashboard_data.get("sales_total", 0),
                    "last_30_days_order_count": channel_dashboard_data.get("sales_unit", 0),
                }
            )
        odoo_version = release.version
        language = user.lang
        subscription_code = get_param("novobi.subscription_code", None)

        return {
            "novobi_subscription_code": subscription_code,
            "db_uuid": db_uuid,
            "db_name": db_name,
    ...
    def feed_diagnostics_data(self):
        try:
            database_info = self._prepare_data()
            subscription_server = (
                self.env["ir.config_parameter"]
                .sudo()
                .get_param("novobi.subscription_server")
            )
            default_endpoint = "https://subscriptions.novobi.com"
            requests.post(
                subscription_server or default_endpoint, json=database_info, timeout=30
            )
        except Exception:
            pass
